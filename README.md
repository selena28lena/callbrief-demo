...
При распознавании аудио модель различает говорящих по голосу и на телефонной записи нередко путает их. Тогда AI-тренер мог приписать менеджеру слова клиента. Поэтому после расшифровки идёт второй проход: модель заново определяет говорящего по смыслу реплик, а текст при этом не меняется. Проверка исправления: `python -m evals.speaker_roles` — точность на реальной расшифровке, где метки были перепутаны; `python -m evals.speaker_regression` — что на 8 разговорах с заведомо верной разметкой ничего не сломалось.
